...
#define PCI_CORE_ACCEL_BASE      0x20000000
#define PCI_CORE_ACCEL_RETVAL    0x20000010
#define PCI_CORE_ACCEL_MASTER0   0x20000018
#define PCI_CORE_ACCEL_MASTER1   0x20000018
#define PCI_CORE_ACCEL_MASTER2   0x20000018
#define PCI_CORE_ACCEL_ARG0      0x20000020
#define PCI_CORE_ACCEL_ARG1      0x20000028
#define PCI_CORE_ACCEL_ARG2      0x20000030
#define PCI_CORE_ACCEL_ARG3      0x20000038
#define PCI_CORE_ACCEL_ARGSTRIDE 0x8
#define PCI_CORE_ACCEL_SYSCALL_ARG0          0x20000040
#define PCI_CORE_ACCEL_SYSCALL_ARGSTRIDE     0x8
#define PCI_CORE_ACCEL_SYSCALL_OUT_ARG0      0x20000070
...
#define PCI_INTC_BASE 0x26000000
...
void juniper_interp_init(struct juniper_fpga_device* dev)
{
	...
	juniper_pci_write(dev->phy_dev, PCI_INTC_IER, intrMask);

	// Set ME and HIE bits in the master interrupt enable
	juniper_pci_write(dev->phy_dev, PCI_INTC_MER, 0x3);
}
...
int juniper_interp_accel_idle(struct juniper_accel_device* dev)
{
	...
	status = juniper_pci_read(dev->fpga->phy_dev, PCI_CORE_ACCEL_BASE | (dev->idx << PCI_CORE_ID_SHIFT));
...
void juniper_interp_accel_start(struct juniper_accel_device* dev)
{
	...
	juniper_pci_write(dev->fpga->phy_dev, PCI_CORE_ACCEL_BASE | (dev->idx << PCI_CORE_ID_SHIFT), status);
}
...
uint32_t juniper_interp_get_arg(struct juniper_accel_device* dev, int argN)
{
	...
	return juniper_pci_read(dev->fpga->phy_dev, addr);
}
...
void juniper_interp_set_arg(struct juniper_accel_device* dev, int argN, uint32_t val)
{
	...
	juniper_pci_write(dev->fpga->phy_dev, addr, val);
}
...
{
	juniper_pci_write(dev->fpga->phy_dev, PCI_CORE_ACCEL_MASTER0 | (dev->idx << PCI_CORE_ID_SHIFT), val);
	juniper_pci_write(dev->fpga->phy_dev, PCI_CORE_ACCEL_MASTER1 | (dev->idx << PCI_CORE_ID_SHIFT), val);
	juniper_pci_write(dev->fpga->phy_dev, PCI_CORE_ACCEL_MASTER2 | (dev->idx << PCI_CORE_ID_SHIFT), val);
}
...
uint32_t juniper_interp_get_interrupt_id(struct juniper_fpga_device* dev)
{
	...
	active_interrupt = juniper_pci_read(dev->phy_dev, PCI_INTC_IVR);
...
void juniper_interp_ack_interrupt(struct juniper_fpga_device* dev, uint32_t intrId)
{
	...
	juniper_pci_write(dev->phy_dev, PCI_INTC_IAR, ackMask);
}